/* 
Maintain the n maximum sums upto ith array, then to get n-maximum sum upto (i + 1)th array use priority queue
*/
#include <bits/stdc++.h>
...
int main(){
    ...
    while(cin>>n){
        ...
        for(int i=1;i<n;++i){
            ...
            for(int i=0;i<n;++i)pq.push({arr[i] + v[0], i}); 
            ...
            while((int)curr.size() < n){
                ...
                pq.pop(); pos[id]++;
                if(pos[id] != n)pq.push({arr[id] + v[pos[id]], id}); 
                curr.push_back(now); 
            }
            v = curr; 
        }
